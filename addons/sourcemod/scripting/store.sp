...
public OnPluginStart()
{
	...
	g_cvarChatTag = RegisterConVar("sm_store_chat_tag", "{lightgreen}[Store]{default} ", "The chat tag to use for displaying messages.", TYPE_STRING);
...
public Native_DisplayConfirmMenu(Handle:plugin, numParams)
{
	...
	Format(m_szCallback, sizeof(m_szCallback), "%d.%d", plugin, callback);
	IntToString(data, m_szData, sizeof(m_szData));
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(g_eCvars[g_cvarCreditAmountKill][aCache])
	{
		...
		if(g_eCvars[g_cvarCreditMessages][aCache])
			CPrintToChat(attacker, "%t", "Credits Earned For Killing", g_eCvars[g_cvarCreditAmountKill][aCache], g_eClients[victim][szName]);
...
public Action:Command_Say(client, const String:command[], argc)
{
	if(argc > 0)
	{
		...
		if(m_szArg[0] == PublicChatTrigger || m_szArg[0] == SilentChatTrigger)
		{
			...
			if (m_szArg[0] == SilentChatTrigger)
			{
...
public Action:Command_Gift(client, params)
{
	...
	new m_iClients = ProcessTargetString(m_szTmp, 0, m_iTargets, 1, 0, m_szTmp, sizeof(m_szTmp), m_bTmp);
	if(m_iClients>2)
	{
		CPrintToChat(client, "%t", "Credit Too Many Matches");
		return Plugin_Handled;
	}
	
	if(m_iClients != 1)
	{
		CPrintToChat(client, "%t", "Credit No Match");
		return Plugin_Handled;
	}
	...
	CPrintToChat(client, "%t", "Credit Gift Sent", m_iCredits, g_eClients[m_iReceiver][szName]);
	CPrintToChat(m_iReceiver, "%t", "Credit Gift Received", m_iCredits, g_eClients[client][szName]);
...
public Action:Command_GiveCredits(client, params)
{
	...
	if(m_iReceiver != -1)
	{
		...
		{
			if(client)
				CPrintToChat(client, "%t", "Credits Given", g_eClients[m_iReceiver][szName], m_iCredits);
			else
	...
	for(new i=0;i<g_iItems;++i)
	{
		if(g_eItems[i][iParent]==parent && (g_eCvars[g_cvarShowVIP][aCache] == 0 && GetClientPrivilege(target, g_eItems[i][iFlagBits], m_iFlags) || g_eCvars[g_cvarShowVIP][aCache]))
		{
			...
			if(g_eItems[i][iHandler] == g_iPackageHandler)
			{
				...
				IntToString(i, m_szId, sizeof(m_szId));
...
public DisplayPlayerMenu(client)
{
	...
	LoopIngamePlayers(i)
	{
		...
		{
			IntToString(g_eClients[i][iUserId], m_szID, sizeof(m_szID));
	...
	if(m_iCount == 0)
	{
		...
		g_iMenuNum[client] = 1;
		DisplayItemMenu(client, g_iSelectedItem[client]);
		CPrintToChat(client, "%t", "Gift No Players");
	}
	else
		DisplayMenu(m_hMenu, client, 0);
}
...
public MenuHandler_Gift(Handle:menu, MenuAction:action, client, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		if(menu == INVALID_HANDLE)
		{
			...
			if(!m_iReceiver)
			{
				CPrintToChat(client, "%t", "Gift Player Left");
				return;
			}
...
public Store_SaveClientInventory(client)
{
	...
	for(new i=0;i<g_eClients[client][iItems];++i)
	{
		strcopy(m_szType, sizeof(m_szType), g_eTypeHandlers[g_eItems[g_eClientItems[client][i][iUniqueId]][iHandler]][szType]);
		strcopy(m_szUniqueId, sizeof(m_szUniqueId), g_eItems[g_eClientItems[client][i][iUniqueId]][szUniqueId]);
		...
		for(new a=0;a<STORE_MAX_SLOTS;++a)
		{
			...
			if(g_eClients[client][aEquipmentSynced][m_iId] == g_eClients[client][aEquipment][m_iId])
				...
				if(g_eClients[client][aEquipment][m_iId]==-1)
					Format(m_szQuery, sizeof(m_szQuery), "DELETE FROM store_equipment WHERE `player_id`=%d AND `type`=\"%s\" AND `slot`=%d", g_eClients[client][iId], g_eTypeHandlers[i][szType], a);
				else
					Format(m_szQuery, sizeof(m_szQuery), "UPDATE store_equipment SET `unique_id`=\"%s\" WHERE `player_id`=%d AND `type`=\"%s\" AND `slot`=%d", g_eItems[g_eClients[client][aEquipment][m_iId]][szUniqueId], g_eClients[client][iId], g_eTypeHandlers[i][szType], a);
...
{	
	...
	g_eClients[client][iCredits] += m_iCredits;
	CPrintToChat(client, "%t", "Chat Sold Item", g_eItems[itemid][szName], g_eTypeHandlers[g_eItems[itemid][iHandler]][szType]);
...
public Store_GiftItem(client, receiver, item)
{
	...
	CPrintToChat(client, "%t", "Chat Gift Item Sent", g_eClients[receiver][szName], g_eItems[m_iId][szName], g_eTypeHandlers[g_eItems[m_iId][iHandler]][szType]);
	CPrintToChat(receiver, "%t", "Chat Gift Item Received", g_eClients[target][szName], g_eItems[m_iId][szName], g_eTypeHandlers[g_eItems[m_iId][iHandler]][szType]);
...
public Store_ReloadConfig()
{
	...
	{	
		new String:m_szFile[PLATFORM_MAX_PATH];
		BuildPath(Path_SM, m_szFile, sizeof(m_szFile), "configs/store/items.txt");
...
Store_WalkConfig(&Handle:kv, parent=-1)
{
	...
	do
	{
		...
		if (KvGetNum(kv, "enabled", 1) && KvGetNum(kv, "type", -1)==-1 && KvGotoFirstSubKey(kv))
		{
			...
			KvGetString(kv, "flag", m_szFlags, sizeof(m_szFlags));
			KvGetString(kv, "games", m_szGame, sizeof(m_szGame));
		...
		else
		{
			...
			KvGetString(kv, "type", m_szType, sizeof(m_szType));
			m_iHandler = Store_GetTypeHandler(m_szType);
			if(m_iHandler == -1)
				continue;
			...
			if(KvJumpToKey(kv, "Attributes"))
			{
				...
				do
				{
					KvGetSectionName(kv, m_szAttribute, sizeof(m_szAttribute));
					KvGetString(kv, NULL_STRING, m_szValue, sizeof(m_szValue));
